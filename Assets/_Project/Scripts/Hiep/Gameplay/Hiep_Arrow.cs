...
public class Hiep_Arrow : MonoBehaviour
{
    ...
    public void SetupArrow(float timeMove, float timeTail, int indexArrow, bool isMustHit,
        float deltaMove, int serial, int sumArrow)
    {
    ...
    public void SetCorrect()
    {
        ...
        Hiep_GameManager.Instance.uiGameplay.SetCorrectArrow(indexArrow);
        Hiep_GameManager.Instance.lsTargetArrows[indexArrow].SetCorrectCollider(indexArrow + 1, timerAnim);
    }
    ...
    public void SetInvisibleTail()
    {
        ...
        Hiep_GameManager.Instance.SetAnimationBoy(0);
    }
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.layer == 8)
        {
            ...
            if (isMustHit)
            {
                ...
                Hiep_GameManager.Instance.lsTargetArrows[indexArrow].SetCollider(this);
            }
            else
            {
                // Set animation for enemy
                Hiep_GameManager.Instance.SetAnimationEnemy(indexArrow + 1);
            }
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.gameObject.layer == 8)
        {
            ...
            if (isMustHit)
            {
                ...
                {
                    // Add score
                    Hiep_GameManager.Instance.AddScore();
                }
                else
                {
                    // Sub score
                    Hiep_GameManager.Instance.SubScore();
                    // Set anim fail for main
                    Hiep_GameManager.Instance.SetAnimationBoy(indexArrow + 5);
                }
                // Exit collider target arrow
                Hiep_GameManager.Instance.lsTargetArrows[indexArrow].ExitCollider(this);
            }
        }
    }
}
