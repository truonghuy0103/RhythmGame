...
namespace Hiep
{
    ...
    public class GameSave
    {
        ...
        public void Init(string version)
        {
            ...
            {
                ModeSaves.Clear();
                for (int i = 0; i < Hiep_ConfigGameplay.GetModeLength(); i++)
                {
                    ModeSave modeSave = new ModeSave();
                    modeSave.IsDownloadAssetBundle = i == 0;
                    ModeSaves.Add(modeSave);

                    for (int j = 0; j < Hiep_ConfigGameplay.GetWeekLength(i); j++)
                    {
                        ModeSaves[i].WeekSaves.Add(new WeekSave());

                        for (int k = 0; k < Hiep_ConfigGameplay.GetSongLength(i, j); k++)
                        {
            ...
            if (ModeSaves.Count < Hiep_ConfigGameplay.GetModeLength())
            {
                for (int i = ModeSaves.Count; i < Hiep_ConfigGameplay.GetModeLength(); i++)
                {
                    ModeSave modeSave = new ModeSave();
